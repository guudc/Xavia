...
const apiMiddleware = (req: Request, res: Response, next: NextFunction): void => {
  ...
  if (!clientKey) {
    return errorResponse(res, StatusCodes.UNAUTHORIZED, "Unauthorized: Missing API key");
  }
  if (clientKey !== process.env.API_KEY) {
    return errorResponse(res, StatusCodes.UNAUTHORIZED, "Unauthorized: Invalid API key");
  }
